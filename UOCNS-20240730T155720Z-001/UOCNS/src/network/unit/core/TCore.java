 ...
 */
package network.unit.core;

import network.TNetworkManager;
import network.common.IConstants;
import network.common.TTypeFlit;
import network.traffic.ICoreTraffic;
import network.traffic.TCoreTraffic;
import network.traffic.TFlit;
import network.traffic.TPacket;
import network.unit.core.buffer.TBufferPacketTx;
import network.unit.router.IRouter;
import network.unit.router.buffer.IBufferPLink;
import network.unit.router.buffer.TBufferPLink;
import network.unit.router.link.TPLinkTx;
...
public class TCore {
    ...
    private Vector<TFlit>[] fArrPacketFlitsRx;
    private int[] fArrCountPacketFlitsToRx;
    private int fVLinkCount;
    private int fSwitchCorePLinkId;
    ...
    private IBufferPLink fBufferPLinkRx;
    private TPLinkTx fPLinkTx;
        ...
        while (iSlot < this.fBufferPacketTx.getSlotCount()) {
            block3:
            {
                ...
                if (!this.fBufferPacketTx.isEmpty(iSlot) && -1 != (iVLinkTxId = this.fBufferPacketTx.getVLinkTxId(iSlot))) {
                    if (!this.fBufferPacketTx.isEmpty(iSlot)) {
                    ...
                    while (this.fPLinkTx.canPushHead(iVLinkTxId, iClock) || this.fPLinkTx.canPushData(iVLinkTxId, iClock)) {
                        ...
                        if (!this.fBufferPacketTx.isEmpty(iSlot)) continue;
                    }
                }
    ...
    public boolean flitPushBackRx(TFlit aFlit, int iClock) throws Exception {
